...
import { PEG_ROWS, plinkoPath, puckAt } from '../lib/plinko.js';
...
function PegField({ cols }: { cols: number }) {
  const rows = Array.from({ length: PEG_ROWS }, (_, r) => r);
  return (
    <>
      {rows.map((r) =>
        Array.from({ length: cols }, (_, c) => {
          // Alternate rows offset by half a column — that's what makes it a peg field.
          const x = c + (r % 2 === 0 ? 0 : 0.5);
          if (x > cols - 1) return null;
          return (
            <span
              // biome-ignore lint/suspicious/noArrayIndexKey: pegs are a fixed positional grid
              key={`${r}-${c}`}
              className="absolute h-[1.1vh] w-[1.1vh] -translate-x-1/2 -translate-y-1/2 rounded-full bg-white/40 shadow-[0_0_6px_rgba(255,255,255,0.25)]"
              style={{
